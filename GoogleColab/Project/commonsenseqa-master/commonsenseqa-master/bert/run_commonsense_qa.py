...
## Required parameters
flags.DEFINE_string(
    "data_dir", '/content/drive/MyDrive/Project/commonsenseqa-master/commonsenseqa-master/data/',
    "The input data dir. Should contain the .tsv files (or other data files) "
    "for the task.")

flags.DEFINE_string(
    "bert_config_file", '/content/drive/MyDrive/Project/commonsenseqa-master/commonsenseqa-master/pre-trained/bert_config.json',
    "The config json file corresponding to the pre-trained BERT model. "
    "This specifies the model architecture.")

flags.DEFINE_string("vocab_file", '/content/drive/MyDrive/Project/commonsenseqa-master/commonsenseqa-master/pre-trained/vocab.txt',
                    "The vocabulary file that the BERT model was trained on.")

flags.DEFINE_string(
    "output_dir", '/content/drive/MyDrive/Project/commonsenseqa-master/commonsenseqa-master/output/',
...
flags.DEFINE_string(
    "init_checkpoint", '/content/drive/MyDrive/Project/commonsenseqa-master/commonsenseqa-master/pre-trained/bert_model.ckpt',
...
def file_based_convert_examples_to_features(
  ...
  for idx, (token_ids, segment_ids, label_ids) in enumerate(
    ...
    for i, (choice_token_ids, choice_segment_ids) in enumerate(
      ...
      features[f'input_ids{i}'] = tf.compat.v1.train.Feature(
        int64_list=tf.compat.v1.train.Int64List(value=list(input_ids.astype(np.int64))))
      features[f'input_mask{i}'] = tf.compat.v1.train.Feature(
        int64_list=tf.compat.v1.train.Int64List(value=list(input_mask.astype(np.int64))))
      features[f'segment_ids{i}'] = tf.compat.v1.train.Feature(
        int64_list=tf.compat.v1.train.Int64List(value=list(segment_ids.astype(np.int64))))

    features['label_ids'] = tf.compat.v1.train.Feature(
      int64_list=tf.compat.v1.train.Int64List(value=label_ids))
...
def file_based_input_fn_builder(
  ...
  name_to_features = {
      "input_ids0": tf.compat.v1.FixedLenFeature([seq_length], tf.int64),
      "input_mask0": tf.compat.v1.FixedLenFeature([seq_length], tf.int64),
      "segment_ids0": tf.compat.v1.FixedLenFeature([seq_length], tf.int64),
      "input_ids1": tf.compat.v1.FixedLenFeature([seq_length], tf.int64),
      "input_mask1": tf.compat.v1.FixedLenFeature([seq_length], tf.int64),
      "segment_ids1": tf.compat.v1.FixedLenFeature([seq_length], tf.int64),
      "input_ids2": tf.compat.v1.FixedLenFeature([seq_length], tf.int64),
      "input_mask2": tf.compat.v1.FixedLenFeature([seq_length], tf.int64),
      "segment_ids2": tf.compat.v1.FixedLenFeature([seq_length], tf.int64),
      "input_ids3": tf.compat.v1.FixedLenFeature([seq_length], tf.int64),
      "input_mask3": tf.compat.v1.FixedLenFeature([seq_length], tf.int64),
      "segment_ids3": tf.compat.v1.FixedLenFeature([seq_length], tf.int64),
      "input_ids4": tf.compat.v1.FixedLenFeature([seq_length], tf.int64),
      "input_mask4": tf.compat.v1.FixedLenFeature([seq_length], tf.int64),
      "segment_ids4": tf.compat.v1.FixedLenFeature([seq_length], tf.int64),
...
def create_model(
  ...
  input_mask = tf.compat.v1.stack(
    ...
    axis=1)
  segment_ids = tf.compat.v1.stack(
    [
      segment_ids0,
      segment_ids1,
      segment_ids2,
  ...
  input_ids = tf.compat.v1.reshape(input_ids, (-1, seq_length))
  input_mask = tf.compat.v1.reshape(input_mask, (-1, seq_length))
  segment_ids = tf.compat.v1.reshape(segment_ids, (-1, seq_length))
  ...
  with tf.compat.v1.variable_scope("loss"):
    ...
    logits = tf.compat.v1.reshape(
      tf.compat.v1.matmul(output_layer, softmax_weights),
      (-1, num_choices))
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.compat.v1.estimator.ModeKeys.TRAIN:
      ...
      def metric_fn(per_example_loss, label_ids, logits):
        predictions = tf.compat.v1.argmax(logits, axis=-1, output_type=tf.int32)
...
def main(_):
  ...
  if FLAGS.do_predict:
    ...
    test_predictions_file = os.path.join(
      FLAGS.output_dir,
      "test_results.csv")
    with tf.compat.v1.gfile.GFile(test_predictions_file, "w") as writer:
